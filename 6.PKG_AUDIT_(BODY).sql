...
BEGIN
	...
	IF nINSERT = 1 THEN
		...
		LOOP
			IF nSAVE_OLD_AND_NEW_VALUES = 1 THEN
				vINSERT:=vINSERT||'NEW_'||c.COLUMN_NAME||', '||chr(10)||chr(13);
			ELSE
	...
	IF nUPDATE = 1 THEN
		...
		LOOP
			IF nSAVE_OLD_AND_NEW_VALUES = 1 THEN
				vUPDATE:=vUPDATE||'NEW_'||c.COLUMN_NAME||', '||chr(10)||chr(13)||
						 		'OLD_'||c.COLUMN_NAME||', '||chr(10)||chr(13);
			ELSE
	...
	IF nDELETE = 1 THEN
		...
		LOOP
			IF nSAVE_OLD_AND_NEW_VALUES = 1 THEN
				vDELETE:=vDELETE||'OLD_'||c.COLUMN_NAME||', '||chr(10)||chr(13);
			ELSE
	...
	UPDATE AUDIT_TABLES_OBJECTS_LIST
		SET TABLE_TRIGGER = UPPER(vTABLE_TRG_NAME),
			INSERT_ADD_CONDITIONS = vINSERT_ADD_CONDITIONS,
			UPDATE_ADD_CONDITIONS = vUPDATE_ADD_CONDITIONS,
			DELETE_ADD_CONDITIONS = vDELETE_ADD_CONDITIONS,
			IS_INSERT = nINSERT,
			IS_UPDATE = nUPDATE,
			IS_DELETE = nDELETE,
			IS_SAVE_OLD_AND_NEW_VALUES = nSAVE_OLD_AND_NEW_VALUES
		WHERE ID = nLIST_AUD_ID;
	COMMIT;
...
BEGIN
	PKG_AUDIT.CREATE_AUDIT_OBJECTS_CUSTOM(vTABLE_NAME				=>vTABLE_NAME,
										...
										nSAVE_OLD_AND_NEW_VALUES	=>nSAVE_OLD_AND_NEW_VALUES);
...
BEGIN
	...
	INSERT INTO AUDIT_TABLES_OBJECTS_LIST(T_SCHEMA,
		...
		VALUES (UPPER(vSCHEMA),
				...
				nUPDATE,
				nDELETE,
				nSAVE_OLD_AND_NEW_VALUES) RETURNING ID INTO nLIST_AUD_ID;
	COMMIT;
		...
		IF nSAVE_OLD_AND_NEW_VALUES = 1 THEN
			...
			LOOP 
				IF cSQL IS NULL THEN 
					cSQL:= c.COLUMN_NAME||' AS NEW_'||c.COLUMN_NAME||', '||c.COLUMN_NAME||' AS OLD_'||c.COLUMN_NAME;
				ELSE
					cSQL:=cSQL||', '||c.COLUMN_NAME||' AS NEW_'||c.COLUMN_NAME||', '||c.COLUMN_NAME||' AS OLD_'||c.COLUMN_NAME;
				END IF;
			END LOOP;
			cSQL:='CREATE TABLE '||AUD_TABLE||' AS (SELECT '||cSQL||' FROM '||SOURCE_TABLE||' WHERE 1=0)';
	...
	ELSE
		...
		IF nSAVE_OLD_AND_NEW_VALUES = 1 THEN
			...
			LOOP 
				IF cSQL IS NULL THEN 
					cSQL:= 't.'||c.COLUMN_NAME||' AS NEW_'||c.COLUMN_NAME||', t.'||c.COLUMN_NAME||' AS OLD_'||c.COLUMN_NAME;
				ELSE
					cSQL:=cSQL||', t.'||c.COLUMN_NAME||' AS NEW_'||c.COLUMN_NAME||', t.'||c.COLUMN_NAME||' AS OLD_'||c.COLUMN_NAME;
				END IF;
			END LOOP;
			...
			LOOP 
				IF cSQL IS NULL THEN 
					cSQL:= c.COLUMN_NAME||' = null';
				ELSE
					cSQL:=cSQL||', '||c.COLUMN_NAME||' = null';
				END IF;
			END LOOP;
			cSQL:= 'UPDATE '||AUD_TABLE||' SET '||cSQL;
			EXECUTE IMMEDIATE cSQL;
		ELSE
...
AS
	...
	INSERT_ADD_CONDITIONS		VARCHAR2(4000);
	UPDATE_ADD_CONDITIONS		VARCHAR2(4000);
	DELETE_ADD_CONDITIONS		VARCHAR2(4000);
	nSAVE_OLD_AND_NEW_VALUES	NUMBER;
BEGIN
	...
	BEGIN
		SELECT IS_INSERT,
				...
				T_SCHEMA,
				TABLE_TRIGGER,
				INSERT_ADD_CONDITIONS,
				UPDATE_ADD_CONDITIONS,
				DELETE_ADD_CONDITIONS,
				IS_SAVE_OLD_AND_NEW_VALUES
			INTO nIS_INSERT,
				nIS_UPDATE,
				nIS_DELETE,
	...
	IF nIS_INSERT <> nINSERT 
		...
		OR nIS_AUTONOMOUS_TRANSACTION <> nAUTONOMOUS_TRANSACTION
		OR (INSERT_ADD_CONDITIONS IS NULL AND vINSERT_ADD_CONDITIONS IS NOT NULL)
		OR (INSERT_ADD_CONDITIONS IS NOT NULL AND vINSERT_ADD_CONDITIONS IS NULL)
		OR (INSERT_ADD_CONDITIONS <> vINSERT_ADD_CONDITIONS)
		OR (UPDATE_ADD_CONDITIONS IS NULL AND vUPDATE_ADD_CONDITIONS IS NOT NULL)
		OR (UPDATE_ADD_CONDITIONS IS NOT NULL AND vUPDATE_ADD_CONDITIONS IS NULL)
		OR (UPDATE_ADD_CONDITIONS <> vUPDATE_ADD_CONDITIONS)
		OR (DELETE_ADD_CONDITIONS IS NULL AND vDELETE_ADD_CONDITIONS IS NOT NULL)
		OR (DELETE_ADD_CONDITIONS IS NOT NULL AND vDELETE_ADD_CONDITIONS IS NULL)
		OR (DELETE_ADD_CONDITIONS <> vDELETE_ADD_CONDITIONS)		THEN
